    ...
    for c in conditions:
        ...
        for p in before_paths:
            ...
            for k, v in df.iloc[0].items():
                if 'Q' in k:
                    if k == 'Q9' or k == 'Q11' or k == 'Q14':
                        v = 100 - v
                    all_ratings.append([c, 'trust-before', k, v])
            ...
            if len(results) > 0 and results[0] > 0:
                ...
                for k, v in df.iloc[0].items():
                    if 'Q' in k:
                        if k == 'Q9' or k == 'Q11' or k == 'Q14':
                            v = 100 - v
                        all_ratings.append([c, 'trust-after', k, v])
            else:
                print('bad result ', p)
    ...
    for c in conditions:
        ...
        for p in paths:
            ...
            if len(results) > 0:
                ...
                for idx, (k, v) in enumerate(df.iloc[0].items()):
                    if 'Q' in k:
                        if idx % 2 != 0:
                            v = 5-v
                        all_ratings.append([c, 'usability', k, v])
                # .tolist()[0:-2]
                # [all_ratings.append(x[0]) for x in df.iloc[0].tolist()]
            else:
                print('bad result ', p)
        data[c] = scores
...
def et_goa_special(d_anomaly, d_help, d_t, d_state, condition):
    ...
    for anomaly, req, t, state in zip(d_anomaly, d_help, d_t, d_state):
        ...
        if anomaly_start < 0 and anomaly != '' and (
                state == 'Executing | Auto | Driving' or state == 'Executing | Auto | Assessing'):
            anomaly_start = t
            if 'b' in anomaly:
                obstacle_type += 'b'
            if 'h' in anomaly:
...
def make_per_run_normalized_csv():
    assmt_correct_rates = {
        'p4': {'correct': 4, 'incorrect': 1},
        'p5': {'correct': 8, 'incorrect': 3},
        'p11': {'correct': 5, 'incorrect': 4},
    ...
    for i in range(22):
        ...
        for p, s in zip(primary_paths, secondary_paths):
            ...
            line = make_csvs(p, s)
            # normalize secondary resp. time to min=0, max=4
            if np.isnan(line[4]):
                line[4] = 0
            if np.isnan(line[7]):
                line[7] = line[3]
            _min = 19.66050363
            _max = 155.15616332
            line[7] = (line[7] - _min) / (_max - _min)

            # normalize secondary skips
            _min = 0
            _max = 3
            line[8] = (line[8] - _min) / (_max - _min)

            # normalize secondary resp. time to min=0, max=4
            _min = 0
            _max = 19
            line[9] = (line[9] - _min) / (_max - _min)

            # normalize mission time to min=127.562708, max=711.707334
            _min = 127.562708
            _max = 711.707334
            line[3] = (line[3]-_min)/(_max-_min)

            # normalize primary resp. time to min=127.562708, max=711.707334
            _min = 0
            _max = 460.999838
            line[4] = (line[4] - _min) / (_max - _min)

            # normalize primary outcome to min=0, max=4
            _min = 0
            _max = 4
            line[5] = (line[5] - _min) / (_max - _min)
            ...
            line.append((line[5] + line[6]) / (1+line[4] + line[7]))
            print(line)
...
def make_survey_csv():
    ...
    for participant, condition in p2c.items():
        try:
            ...
            # Surveys
            trust_score, trust_ratings = plot_trust_survey_responses(experimental_conditions, base=base, show=False)
            usability_score, usability_ratings = plot_usability_scores(experimental_conditions, base=base, show=False)
            ...
            BEFORE, AFTER = 0, 1
            if participant == 'p2':
                trust_score[condition][BEFORE] = trust_score[condition][AFTER]
            line = [participant, condition, trust_score[condition][BEFORE][0], trust_score[condition][AFTER][0],
                    (trust_score[condition][AFTER][0] - trust_score[condition][BEFORE][0]) / abs(
                        trust_score[condition][BEFORE][0]),
                    usability_score[condition][0],
                    ages[0], robotics_exp[0], gaming_exp[0], gender[0]]
            lines.append(line)
            [raw_lines.append(x) for x in usability_ratings]
            [raw_lines.append(x) for x in trust_ratings]
    ...
    df = pd.DataFrame(data=raw_lines, columns=['condition', 'type', 'Question', 'Response'])
    df.to_csv('surveys_raw.csv', index=False)


if __name__ == '__main__':
    #make_survey_csv()
    make_per_run_normalized_csv()
    #make_per_run_csv()
    print(0 / 0)
    base = '../data/mixed/'
    experimental_conditions = ['TELEM', 'GOA', 'ET-GOA']
